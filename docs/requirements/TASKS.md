...
- **验证结果**：
  ...
  ② 冒烟 `node test/ui-smoke.cjs` 在 `model/蹲姿.glb` 与 `o-model/蹲姿.ive` 上**各 32 步、0 项失败**（既有 36 条 check 一条未删，现共 94 条；新增布局/键盘断言）；默认布局实测左 260 / **中 492（最宽）** / 右 340、3D 画布 492×274、无整页滚动。
  ③ 独立冷上下文复审结论 **通过（无阻塞级、无重要问题）**，其 6 条次要项已逐条处理：坏值不再退化成下限（`typeof value !== 'number'` 才回落默认）、`localStorage` 的 `version` 不匹配整份丢弃、冒烟里那条自我满足的默认布局断言换成"种入非默认值 + 重载 + 必须真的生效"、`resizeCount()` 只统计真实调用、窄布局的体检配色线索由状态栏接力、底部栏上限再扣预览控件条高度以保画布 ≥160px。
