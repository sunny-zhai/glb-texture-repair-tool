...
- **状态**：**已完成（交付闸门 ③ · sunny-zhai · 2026-09-22 · 批准，含一处已知偏差）**（闸门 ① 规格 · sunny-zhai · 2026-09-20；闸门 ② 架构 · ADR-012 · 2026-09-20。**先做 spike 再定架构**）。TASK-027（spike 判 WASM 可行）→ TASK-028（WASM 助手落地并修掉打包后 asar 路径缺陷）→ TASK-029（BR-036 与四平台发布清单回填）→ TASK-030（mac/linux 打包目标与平台矩阵）→ TASK-031（冷审返工：发布形态收窄为**只带 WASM** 使**标准 2** 真正成立、补 deb 元数据使 **`dist:linux`** 能产出、原生助手起不来时回退 WASM、`dist:linux` 固定 `--x64`）→ TASK-032（复核收口：both-fail 错误可读性、`helperKind` 语义、台账口径）。已实测：darwin-arm64 打包后**应用内** `resolveIveHelper()` 报 `kind: wasm` 且产物与开发态原生助手**逐字节相同**；`electron-builder --linux --x64` 产出 AppImage(x86_64) + deb(amd64) 且 `Maintainer`/`Vendor`/`Homepage` 齐备；包内 `find -type f -name 'ive2glb*'` 只命中 wasm 两个文件。**已知偏差（批准时明确接受）**：验收标准 1 的「四平台**各自实测**」只有 `darwin-arm64` 完成，`linux-x64` 只到「安装包产出且内容/元数据正确」，`win32-x64` / `darwin-x64` 未做——该偏差由发布执行的 `docs/release/RELEASE_CHECKLIST.md` §3/§4 ★ 行承接，是**发布前置**而非本需求未完成；另逐字节等价性是单 IVE 夹具（`o-model/蹲姿.ive`）结论
- **优先级**：P1
...
- **确认**：**已确认**（闸门 ① 规格 · sunny-zhai · 2026-09-20；闸门 ② 架构 · ADR-012 · 2026-09-20；**交付闸门 ③ approved · sunny-zhai · 2026-09-22**，含「标准 1 四平台实测」的已知偏差，见 `docs/approvals/APPROVALS.md`。同批 REQ-009 的取代关系以 abandon 留痕）
...
| 2026-09-22 | REQ-012 | 交付闸门 ③ 批准（含已知偏差） | sunny-zhai 批准交付：TASK-027~032 全部交付，两轮冷上下文复核为 PASS / CONDITIONAL PASS 且剩余 minor 已由 TASK-032 收口。**明确接受一处偏差**——验收标准 1 的「四平台各自实测」只有 darwin-arm64 完成，linux-x64 只到「安装包产出且内容/元数据正确」，win32-x64 / darwin-x64 未做；该偏差由发布执行的 §3/§4 ★ 行承接，是发布前置而非需求未完成。状态由「已确认」改为「已完成」，留痕见 `docs/approvals/APPROVALS.md` |
