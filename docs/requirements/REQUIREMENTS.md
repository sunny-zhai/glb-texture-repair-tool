...
- **验收标准**（Given/When/Then）：
  ...
  6. **内部元素自适应**：Given 从 900×700 到 1920×1200 的尺寸矩阵，When 逐区检查，Then ① 每个区域内"overflow 为 auto|scroll 的元素"仍**恰好 1 个**（BR-029 不回归）；② 无横向溢出（每区 `scrollWidth <= clientWidth + 1`）；③ 中栏 3D 画布高度 ≥ `CANVAS_MIN_HEIGHT`（长路径/窄宽度下预览信息行用省略号而不是换行撑高）；④ 状态栏与预览控件条不换行、不被裁切；⑤ 页面本身不整页滚动（`scrollHeight <= innerHeight + 1`）。
