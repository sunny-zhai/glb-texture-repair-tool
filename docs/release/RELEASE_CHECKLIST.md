...
- [x] 关联 REQ 已验收：REQ-001（TASK-001/002/003）、REQ-002（TASK-004）、REQ-003/REQ-004（追溯登记）、REQ-005（TASK-007 经两轮独立复审关闭，TASK-008/009 已完成，**人工目视 2026-09-20 通过**）、REQ-006（TASK-010~012 已完成，**人工目视 2026-09-20 通过**）、REQ-007（TASK-013~016 已完成，**人工目视 2026-09-20 通过**）、REQ-008（TASK-017~020 全部完成，11 条验收标准有自动化证据）、REQ-010（TASK-023 完成）、REQ-011（TASK-024~026 完成）、REQ-012（TASK-027~031 全部完成：WASM 助手落地、四平台打包目标、BR-036 与清单回填、发布形态收窄为**只带 WASM** 并补齐 deb 元数据；打包应用内实测 `kind: wasm` 且产物与原生逐字节相同。**验收标准 1 的四平台"实际安装与运行"仍待各自环境回填**，见 §3/§4 的 ★ 行）、REQ-009（TASK-021/022 **已取消（被 REQ-012 取代）**，并经交付闸门 ③ 以 abandon 留痕，见 §6）
- [x] 全量测试通过：`npm test` → **155 用例 / 149 通过 / 0 失败 / 6 跳过**（**2026-09-22 TASK-032 后复测**；本地跳过数**按当次夹具集陈述**：当前本地语料只剩 `o-model/蹲姿.*`、`model/` 为空，6 个跳过 = `o-model/运输车.glb` 1 + `model/蹲姿.glb` 1 + `model/person-stand.glb` 1 + `o-model/person-move.ive` 3；TASK-031 前的 148 用例夹具齐备时为 144 通过 / 4 跳过）；**全新克隆（无任何夹具）实测 155 用例 / 131 通过 / 0 失败 / 24 跳过**（`git worktree add --detach` 干净检出 + 软链 `node_modules`，跳过 24 = `repair` 7 + `ive` 9 + `inspect` 3 + `convert` 5；TASK-032 新增的 3 条自建桩环境，不依赖夹具）；`npm run lint` 通过
- [x] 覆盖率已**实测并记录**：`all files` 行 **95.38%** / 分支 **81.22%** / 函数 **96.82%**（**2026-09-22 TASK-032 后复测**；`convert.js` 94.55%、`inspect.js` 94.31%、`ive.js` 95.62%、`repair.js` 90.87%、`report-format.js` 99.35%、`transform.js`/`preview-transform.js` 100.00%；分支率有单次运行 ±0.1pp 的波动）。**未设强制门槛**（无插桩门槛、无 CI），Electron 壳层未纳入插桩——详见 `docs/testing/TEST_PLAN.md` 覆盖率一节。
  **测量命令必须带 `--test-coverage-exclude="vendor/**"`**：`node --test --experimental-test-coverage --test-coverage-exclude="vendor/**" "test/*.test.js"`。原因是 TASK-028 起 `vendor/ive2glb/wasm/ive2glb.js`（Emscripten 生成的胶水代码）会被子进程继承的 `NODE_V8_COVERAGE` 插桩——它是随包分发的第三方产物，不是本项目源码；不排除会把 `all files` 函数覆盖率从 96.82% 拖到 47.99%，得出没有意义的数字
- [x] 独立审查（reviewer 冷上下文）已通过：REQ-005 的 TASK-007 经**两轮**独立复审（第一轮不通过 → 修复 → 第二轮「有条件通过」，2 个阻塞级 + 11 项残留缺陷已修）、REQ-006 的 TASK-010 经**两轮**冷审（画布最小高度未兑现等已修）、REQ-007 的 TASK-016 修掉冷审 6 条重要项（含 3 条可复现反例）、**REQ-012 的 TASK-031 经两轮冷审返工后，又由两名独立 verifier 复核为 PASS / CONDITIONAL PASS**（见 `docs/requirements/TASKS.md` 的返工与收口记录）。**闸门状态**：REQ-008 / REQ-009 的规格闸门 ① 与 ADR-009/ADR-010 的架构闸门 ② **均已批准**（`docs/approvals/APPROVALS.md`）——本条此前误写"待确认"，2026-09-22 订正
