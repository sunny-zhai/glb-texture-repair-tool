...
**总计**（2026-09-20 实测，含 REQ-011 与 REQ-010）：`npm test` → **141 用例 / 137 通过 / 0 失败 / 4 跳过**。4 个跳过均为夹具门控（1 个需 `o-model/运输车.glb` 的体检真值用例 + 3 个需 `o-model/person-move.ive` 的 IVE 用例）。全新克隆（无任何夹具）为 **141 用例 / 121 通过 / 0 失败 / 20 跳过**（20 = `repair` 7 + `ive` 5 + `inspect` 3 + `convert` 5；用 `git worktree add --detach HEAD` 的干净检出逐文件实测）。冒烟另计：`node test/ui-smoke.cjs` → **66 步 / 132 条断言 / 0 失败**（TC-024 的占比与多尺寸断言把计数从 114 抬到 126，TC-025 的预览记忆断言再抬到 132；"36 步 / 114 条断言"是 TASK-018 时代的口径）。
