...
- **项目文档**：REQUIREMENTS.md ✓ · TASKS.md ✓ · TEST_PLAN.md ✓ · PERF_BUDGET.md ✓ · RELEASE_CHECKLIST.md ✓ · APPROVALS.md ✓ · ADR.md ✓ · openapi.json 不适用 · PROJECT_MEMORY.md ✓
...
| 2026-09-22 | REQ-012 | TASK-029 | completed | 新增 BR-036（WASM 优先/原生仍优先/asar.unpacked 优先/.wasm+.js 成对）与 §6 第 22 条；MOD-005 双形态、MOD-006 补 resolveIveHelper 顺序；RELEASE_CHECKLIST §3/§4 改四平台口径并解除「推迟发布」；TEST_PLAN 补 TC-026/新增 TC-027 与覆盖率排除命令；CLAUDE.md IVE 章节。本地跳过数改为随夹具集陈述。验证：lint 通过；npm test 148/142/0/6；干净检出 148/126/0/22；覆盖率 all files 95.32/81.19/96.72；memory check 通过 | ffef852 |
| 2026-09-22 | REQ-012 | TASK-031 | completed | 冷审返工：files/asarUnpack 收窄为 vendor/ive2glb/wasm/**（包内 find -type f -name 'ive2glb*' 只命中 wasm 两文件，app.asar 里 darwin-arm64 出现 0 次）→ 标准 2 成立；补 author.email/homepage/repository/linux.maintainer 后 electron-builder --linux --x64 实跑产出 AppImage(x86_64)+deb(amd64) 且 Maintainer/Vendor/Homepage 齐备；runHelper 标记 launchFailure、convertIveToGlb 回退 resolveWasmHelper（helperKind/report.warnings 记录），新增 4 条用例；searched 去重；ive/convert/inspect 跳过信息补恢复命令；BR-036 交付形态与开发态顺序分开写、ADR-012 标 101.6ms 为起助手耗时（全链路 0.55~0.65s）；台账订正 CLAUDE.md 13→14、TEST_PLAN 各套件计数与待执行段。打包应用内 resolveIveHelper → kind wasm，convertIveToGlb(蹲姿.ive) → success 0.538×1.364×1.056/11516/18924/2544480B 与原生 cmp 逐字节相同。验证：lint 通过；npm test 152/146/0/6；干净检出 152/128/0/24；覆盖率 all files 95.39/81.23/96.78；memory check 通过。未验证：win32-x64/linux-x64/darwin-x64 的实际安装与运行 | ad79246 |
| 2026-09-22 | REQ-012 | TASK-032 | completed | 复核收口（两轮独立复核只剩 minor）：src/ive.js 新增 shortenForError（长 stderr 截断+标注，冷审实测错误串 65,792 字符）、both-fail 时错误串带出原生失败原因、report.helperKind 改为解析后立即写（失败=最后尝试的形态）、失败路径带出 iveWarnings；test/ive.test.js 新增 3 条不依赖夹具的用例（shortenForError、both-fail 错误串、无 WASM 兜底时 helperKind）；BR-036 与 MOD-006、CLAUDE.md 的措辞同步；TEST_PLAN TC-001 期望行订正为 36/35/1、TC-002 38/35/3、总计 155/149/0/6、干净检出 155/131/0/24、覆盖率 95.38/81.22/96.82（ive.js 95.62/72.54/98.44）；RELEASE_CHECKLIST §1 数字同步并修正失实的「REQ-008/009 闸门待确认」，REQ-012 预检补 TASK-030/031。验证：lint 通过；npm test 155/149/0/6；memory check 通过 | 16dad84 |
