...
| BR-018 | 体检必须**自算世界盒**（沿节点链累乘矩阵后重新包角点），并同时给出 accessor 并集盒与两者的偏差倍数；偏差 > 10 倍即告警。实测装甲救护车 229,713 倍、运输车 4,461 倍、person-move 485 倍 | P0 |
| BR-019 | 上轴只做**保守推断**并给出置信度与依据：有导出器签名（FBX2glTF/assimp/Khronos/Blender）→ Y 轴 medium；仅凭包围盒 → unknown/low 并交人工确认。绝不静默改写模型朝向 | P0 |
| BR-020 | 体检**只读**且**不抛异常**：文件不可读、非 GLB、解析失败、缺 accessor `min`/`max`、外部贴图缺失等一律转成报告里的中文问题条目（`level`/`code`/`message`），命令行退出码反映成败 | P0 |
